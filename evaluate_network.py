...
def first_player_point(ended_state):
    # 1: 선 수 플레이어 승리, 0: 선수 플레이어 패배, 0.5: 무승부
    if ended_state.is_lose():
        return 0 if ended_state.first_player else 1
...
def play(next_actions):
    ...
    while True:
        ...
        # 행동 얻기
        next_action = next_actions[0] if state.first_player else next_actions[1]
...
def evaluate_network():
    # 최신 플레이어 모델 로드
    model0 = tf.keras.models.load_model('./model/latest.h5')
